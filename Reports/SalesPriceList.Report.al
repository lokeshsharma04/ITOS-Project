report 52146 "AME Sales Price List Report"
{
    ApplicationArea = All;
    Caption = 'Sales Price List - Report';
    UsageCategory = ReportsAndAnalysis;
    DefaultRenderingLayout = SalesPriceListHistory;
    dataset
    {
        dataitem(PriceListHeader; "Price List Header")
        {
            DataItemTableView = sorting(Status, "Price Type", "Source Group", "Source Type", "Source No.", "Currency Code", "Starting Date", "Ending Date") where("Price Type" = const(Sale), "Source Group" = const(Customer));
    ...
    rendering
    {
        layout(SalesPriceListHistory)
        {
            Caption = 'Sales Price List History';
            Type = Excel;
            LayoutFile = 'src/Reports/Layout/SalesPriceListHistory.xlsx';
        }
    }
}
