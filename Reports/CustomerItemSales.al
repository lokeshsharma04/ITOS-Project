report 52135 "AME Customer/Item Sales"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'src/Reports/Layout/Customer Item Sales.rdl';
    ApplicationArea = Basic, Suite;
    Caption = 'Customer/Item Sales with Cost';
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            dataitem("Value Entry"; "Value Entry")
            {
                ...
                begin
                    ...
                    if "Value Entry"."Sales Amount (Actual)" > 0 then
                        TempValueEntryBuffer."Cost per Unit" := Abs("Value Entry"."Sales Amount (Actual)" / "Value Entry"."Invoiced Quantity")
                    else
                        if TempValueEntryBuffer."Sales Amount (Actual)" = 0 then
                            TempValueEntryBuffer."Cost per Unit" := 0
                        else
                            TempValueEntryBuffer."Cost per Unit" := "Value Entry"."Sales Amount (Actual)" / "Value Entry"."Invoiced Quantity";
                    //Cost Amt. Custom
            ...
            dataitem("Integer"; "Integer")
            {
                ...
                column(CostAmt; (TempValueEntryBuffer."Cost Amount (Actual)" + TempValueEntryBuffer."Cost Amount (Non-Invtbl.)" * -1)) { }
                trigger OnAfterGetRecord()
                begin
                    if Number = 1 then
                    ...
                    if ShowBelowCost AND (Profit > 0) then
                        CurrReport.Skip();
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(ShowBelowCost; ShowBelowCost)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Show Only Below Cost';
                    }
